...
def url_fix(s, charset='utf-8'):
    ...
    return urlparse.urlunsplit((sche1me, netloc, path, qs, anchor))
...
# Parse the command
    ...
    qw = 'Immigration'   # Actual query word(s)
    ge = ''
...
def tw_search(api):
    ...
    csvFile = open('result_immig.csv','w',encoding='utf-8')
    #Use csv Writer
    ...
    for tweet in tweepy.Cursor(api.search,
		...
		# Dongho 03/28/16
        csvWriter.writerow([created, str(text).encode('utf-8',errors='ignore'), retwc, hashtag, followers, friends])
        #csvWriter.writerow([created, str(text).encode('ascii','ignore'), retwc, hashtag, followers, friends])
        counter = counter +1
        if (counter == c):
            break
